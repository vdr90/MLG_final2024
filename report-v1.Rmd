...
Este trabajo pretende efectuar un análisis de la popularidad de canciones en base a diferentes variables predictoras inherentes a las canciones. 

Si bien, existen muchas bases de datos que proporcionan información acerca de artistas y piezas musicales, nos concentraremos en una base de datos de spotify que nos brinda dicha información.

Spotify es una de las plataformas de streaming de música más grandes y populares a nivel mundial. 

* Número de usuarios: A partir de principios de 2024, Spotify reportó tener más de 550 millones de usuarios activos mensuales, con más de 240 millones de suscriptores de pago. Esto lo posiciona como una de las plataformas de música más utilizadas a nivel global.

* Cobertura geográfica: Spotify está disponible en 184 países y territorios, lo que le proporciona una presencia global significativa. Esto incluye la mayoría de los principales mercados de música en América del Norte, Europa, América Latina, Asia y África.

* Biblioteca: Spotify cuenta con una biblioteca de más de 100 millones de canciones y 5 millones de podcasts cubriendo una amplia variedad de géneros y estilos.

* Influencia en la industria musical: Spotify tiene un impacto considerable en la industria musical. Las listas de éxitos y las playlists hechas por Spotify, pueden influir significativamente en el éxito de una canción o artista.

* Audiencia demográfica: La audiencia de Spotify tiende a ser más joven, con una gran proporción de usuarios en el rango de 18 a 34 años.


Bajo este alcance, sabemos que la popularidad de una canción, no siempre es reflejo de su calidad, popularidad a largo plazo, o popularidad afuera de la audiencia de Spotify. 

Además del artista, hay otras características que hacen popular a una canción, como el género, tempo o la “valencia”, que define qué tan alegre suena una pieza.

En este trabajo intentaremos entender por que algunas canciones son más populares que otras, para esto definimos a la variable $Y$ como un rating de popularidad con $Y ∈ [0, 100]$, y sabemos que, en general, entre más reproducciones recientes tenga una una canción, su rating de popularidad será mayor.
...
3.  Modelo con parámetros homogéneos: Modelaremos la popularidad de las canciones tomando en cuenta una medida general de la popularidad de los artistas, es decir, no tomaremos en cuenta la popularidad individual de cada artista.
4.  Modelo con parámetros heterogéneos: EN contraste al modelo anterior, modelaremos la popularidad de las canciones tomando en cuenta la popularidad individual de los artistas, permitiendo que este parámetro varie entre artista y artista.
5.  Modelo jerárquico: Plantearemos un modelo de jerarquía que nos permita integrar  los dos enfoques anteriores, permitiendo que exista variabilidad entre la popularidad del artista, pero teniendo en cuenta la popularidad general que reepresenta la interacción entre todos los artista en la base de datos. FInalmente, compararemos los resultados entre los dos modelos anteriores y este con el fin de determinar que modelo ajusta mejor a los datos.
6.  Modelo jerárquico con DAG: Haremos un DAG para agregar la variable de valencia al modelo jerárquico con el onjetivo de observar que tanto influye esta variable en la popularidad general de una canción.
...
datos <- subset(datos, duration_ms != 0)

selected_datos_small <- read.csv("datos/datos_small.csv") 
selected_datos_small <- subset(selected_datos_small, duration_ms != 0)
selected_datos_small <- subset(selected_datos_small, popularity != 0)


selected_datos <- read.csv("datos/selected_datos_75.csv") 
selected_datos <- subset(selected_datos, duration_ms != 0)
selected_datos <- subset(selected_datos, popularity != 0)
...
Tenemos 114 mil registros de canciones y 21 características que los representan, entre las cuales destaca el identificador de la canción, el artista, el álbum en el que fue publicada, la popularidad, su duración, entre otras características. Debido al limitado poder de procesamiento y performance, así como el tiempo de entrenamiento de los modelos, optamos por reducir la cantidad de registros y utilizar sólo una muestra de 75,000 canciones. Nos enfocaremos en el análisis y modelado de la popularidad, sin embargo, en el anexo 1 de este trabajo se puede consultar la limpieza y adecuación de los datos utilizados.
...
* $\epsilon_i$​ es el error aleatorio asociado con la observación $i$, que asumiremos normalmente distribuido con media cero y varianza constante.

Para nuestro ejercicio requerimos 5 betas para cada una de las variables predictoras que usaremos.

```{r}

# Convertimos género a factor (tratamiento de categóricas)
selected_datos_small$genre_numeric <- as.factor(selected_datos_small$genre)

```

Stan no maneja factores directamente, así que necesitamos convertir la variable factor genre en variables dummy (indicadoras). 
Podemos hacer esto utilizando la función model.matrix().

```{r}

# Crear variables dummy para 'genre'
genre_dummies <- model.matrix(~ genre_numeric - 1, selected_datos_small)

# Verificar el resultado
head(genre_dummies)

#Añadimos las dummies
selected_datos_small <- cbind(selected_datos_small, genre_dummies)

```

```{r}

#Adicionalmente cambiamos duración a segundos

selected_datos_small$duration <- selected_datos_small$duration_ms / 1000
...
fit_lineal <- sampling(stan_model_lineal, 
                       data = list(N = nrow(selected_datos_small), 
                                   ...
                                   duration = selected_datos_small$duration,
...
-   La duración parece tener una muy ligera influencia en la popularidad, ya que su coeficiente es cercano a cero.
...
-   El género parece tener una influencia considerable en la popularidad, lo que sugiere que ciertos géneros podrían ser más populares en promedio en comparación con otros.
...
En nuestro modelo anterior, las priors eran poco  informativas ya que acotamos la variabilidad. EN este segundo modelo integraremos priors más informativas que reflejen nuestro conocimiento de las variables:

-   ¿Cuánto dura la canción?: duration
...
-   Tempo:
    ...
    Variable continua entre 0 y 244. Utilizaremos una distribución uniforme.

    $$tempo \sim U(0,244)$$

-   Género:

    El género es una variable entera categórica, por lo que la modelaremos usando una distribución Normal.

    $$género \sim N(\mu, \sigma)$$
...
```{r message= FALSE, results="hide"}
  ...
  parameters {
    ...
    real<lower=0, upper=244> p_tempo;               
    real p_genre;               
  ...
  model {
    ...
    for (i in 1:N) {
      ...
      popularity[i] ~ neg_binomial_2(p_duration * duration[i] + 
                      p_explicit * explicito[i] +
                      p_dance * danceability[i] +
                      p_tempo * tempo[i] +
                      // dot_product(p_genre, to_vector(genre_dummies[i]));
                      p_genre * genre_dummies[i],
                      // p_genre * genre[i];
                      phi_popularity); // Distribución binomial negativa para popularidad
    }
  }
  "
...
Podemos deducir que la explicitividad, la danzabilidad, el tempo y especialmente el género parecen tener influencias significativas en la popularidad de una canción, mientras que la duración parece tener una influencia insignificante. 

Específicamente, las canciones más explícitas, adecuadas para bailar y con ciertos tempos, tienden a ser más populares en promedio, en cuanto al género, requeriremos hacer un análisis más detallado pues si bien es una variable de afectación considerable, debemos plantear otro esquema para determinar que géneros son los que impactan más.
...
Para observar el desempeño obtendremos las prediccines de popularidad para 20 canciones y compararemos estas con la popularidad real.
